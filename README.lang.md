# GitMergeFlow - Language Selection

<div align="center">

![GitMergeFlow Logo](https://img.shields.io/badge/GitMergeFlow-1.0.2-blue.svg)
![Platform](https://img.shields.io/badge/platform-IntelliJ%20IDEA-orange.svg)
![License](https://img.shields.io/badge/license-MIT-green.svg)
![Java Version](https://img.shields.io/badge/java-21%2B-brightgreen.svg)
![IDEA Versions](https://img.shields.io/badge/IDEA-2025.2%2B-informational.svg)

**🔀 智能Git分支合并工具 / Intelligent Git Branch Merging Tool**

[![Stars](https://img.shields.io/github/stars/yanxianchao/gitmergeflow?style=social)](https://github.com/yanxianchao/gitmergeflow/stargazers)
[![Forks](https://img.shields.io/github/forks/yanxianchao/gitmergeflow?style=social)](https://github.com/yanxianchao/gitmergeflow/network)
[![Issues](https://img.shields.io/github/issues/yanxianchao/gitmergeflow)](https://github.com/yanxianchao/gitmergeflow/issues)
[![Downloads](https://img.shields.io/github/downloads/yanxianchao/gitmergeflow/total)](https://github.com/yanxianchao/gitmergeflow/releases)

</div>

---

## 🌐 Language / 语言

Please select your preferred language to view the documentation:
请选择您的首选语言查看文档：

### 🇨🇳 简体中文 / Simplified Chinese

**[查看简体中文文档 →](README.md)**

GitMergeFlow 是一款专为 IntelliJ IDEA 设计的智能Git分支合并插件，旨在解决多项目开发环境中的代码同步难题。通过在Git推送对话框中集成智能合并功能，实现一键式自动合并，大幅提升开发效率。

---

### 🇺🇸 English

**[View English Documentation →](README.en.md)**

GitMergeFlow is an intelligent Git branch merging plugin designed specifically for IntelliJ IDEA, aimed at solving code synchronization challenges in multi-project development environments. It achieves one-click automatic merging by integrating intelligent merging functionality into the Git push dialog.

---

## 📝 Version / 版本

### v1.0.2 (2025-02-13)
**Platform Upgrade / 平台升级**:
- IntelliJ Platform Gradle Plugin 2.10.2
- Kotlin 2.1.20
- Java 21
- Supports IntelliJ IDEA 2025.2+ / 支持 IntelliJ IDEA 2025.2+

## 📊 Project Overview / 项目概览

| Language / 语言 | File / 文件 | Description / 描述 |
|-----------------|-------------|-------------------|
| 🇨🇳 简体中文 | [README.md](README.md) | 默认文档，包含完整的功能介绍和使用指南 |
| 🇺🇸 English | [README.en.md](README.en.md) | English version for international users |

---

## 🚀 Quick Links / 快速链接 / 快速連結

### Installation / 安装

- [Manual Installation](https://github.com/yanxianchao/gitmergeflow/releases) - Download latest release

### Documentation / 文档

- 🇨🇳 [简体中文文档](README.md)
- 🇺🇸 [English Documentation](README.en.md)

### Development / 開發 / 开发

- [GitHub Repository](https://github.com/yanxianchao/gitmergeflow)
- [Issues](https://github.com/yanxianchao/gitmergeflow/issues)
- [Discussions](https://github.com/yanxianchao/gitmergeflow/discussions)

---

## 📞 Contact / 联系

- **Author / 作者**: yanxianchao
- **Email**: xianchao.yan@qq.com
- **GitHub**: [@yanxianchao](https://github.com/yanxianchao)

---

<div align="center">

**⭐ If this project helps you, please give us a Star!**
**⭐ 如果这个项目对您有帮助，请给我们一个Star！**

Made with ❤️ by [yanxianchao](https://github.com/yanxianchao)

</div>